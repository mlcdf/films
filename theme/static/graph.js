...
const groupBy = {
    "year": {
        ...
        tickFormat: i => new Date(Date.UTC(i, 0, 0)).toLocaleString("fr-FR", { year: "numeric" }),
        transform : x => x,
        thresholds: (n) => n - n % 10
    },
    "rating": {
        label: "Note",
        transform : x => (x - 0.25) / 2,
    },
    "done_date": {
...
const draw = function (x, group, filter) {
...
document.querySelector("select").addEventListener("change", e => {
    draw(e.target.value, groupBy[e.target.value], "");
})

draw("rating", groupBy["rating"])

const chart = Calendar(data.filter(d => d.done_date != undefined), {
    x: d => new Date(d.done_date.replace("-00", "-01")),
    y: d => d.title,
})
